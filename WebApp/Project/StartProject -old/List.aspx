<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="WebApp.Project.StartProject.List" %>
...
<script type="text/javascript">
    ...
    cfg.columns = [
        ...
        { field: "Quota", name: "批复金额(万元)", width: "15%", align: "left", render: cfg.render, drag: true },
    ...
    var returnPro = function () {
        ...
        $.ajax({
            ...
            data: { action: "return", proguid: ids, userguid: userguid, username: username, returninfo: returninfo },
            cache: false,
            async: false,
            dataType: "json",
    ...
    var sucessPro = function () {
        //获取当前proguid、userguid、username
        //var proguid = $("#grid").GetSelection("total");
        var userguid = '<%=CurrentUser.UserGuid%>';
        var username = '<%=CurrentUser.UserCN%>';
        ...
        $.ajax({ 
            type: "post", 
            url: "WebFunction.ashx",
            data: { action: "submit", proguid: ids, userguid: userguid, username: username },
            cache:false, 
            async:false, 
            dataType: "json", 
            success: function(obj){ 
    ...
    function ttabs(pg, titile) {
        if ($('.easyui-tabs').tabs('exists', titile)) {
            ...
            $('.easyui-tabs').tabs('add', {
                title: titile,
                content: '<iframe src="../ProjectForm.aspx?Guid=' + pg + '" frameborder="0" width="100%"  height="' + (h + 10) + '"></iframe>',
                closable: true
            });
        }
    }
