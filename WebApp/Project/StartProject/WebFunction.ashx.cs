    ...
    public class WebFunction : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            else if (action == "namevalid")
            {
                ...
                if (projname != "")
                {
                    string querysql = "select count(*) as procount from tz_Project where sysstatus!=-1 and  proguid!='" + proguid + "' and  proname='" + projname + "'";
        ...
        private void commitPro(HttpContext context)
        {
            ...
            try
            {
                ...
                string sql = " insert into tz_projecthistory(Guid,ProGuid,ProAction,InfoData,CreateDate,CreateUserGuid,CreateDepGuid) values(newid(),@ProGuid,@ProAction,@InfoData,getdate(),@CreateUserGuid,@CreateDepGuid)";
                var dbcmd = db.CreateCommand(System.Data.CommandType.Text, sql);
                db.AddInParameter(dbcmd, "@ProGuid", System.Data.DbType.String, proGuid.Replace("'", ""));
                ...
                {
                    sql2 = " update tz_Project set ProState='提交' where (ProState is null or ProState='暂缓')  and ProGuid in(" + proGuid + ")";
                }
                else
                {
                    sql2 = " update tz_Project set ProState='申报' where (ProState is null or ProState='暂缓')  and ProGuid in(" + proGuid + ")";
                }
