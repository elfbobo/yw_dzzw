...
namespace WebApp.Project.tz_ProjOverview
{
    public partial class Index : SharedPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                tableTitle = "部门项目概况";
                tjquerysql = "select count(*) as xmcount,sum(Quota) as total_ys,sum(htje) as total_htje,sum(zfje) as total_zfje,startdeptname from V_TZ_ProjectOverview  group by startdeptname,sortnum order by sortnum";
            ...
            else
            {
                ...
                DataSet dsprotj = db.ExecuteDataSet("select * from V_TZ_ProjectOverview where startdeptguid='" + CurrentUser.UserGroup.Guid + "' order by startdate desc,sortid desc");
                tableTitle = "项目信息";
