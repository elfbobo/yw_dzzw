        ...
        init: function (options, swfUploadOptions) {
            return this.each(function () {
                ...
                var settings = $.extend({
                    ...
                    buttonText: '文件选择',     // The text to use for the browse button
                ...
                if (args.length === 1) {
                    ...
                    switch (name) {
                        ...
                        case 'buttonCursor':
                            ...
                            break;
                        case 'buttonText':
                            $('#' + settings.id + '_button').find('.uploadify-button-text').html(value);
                            break;
                        case 'width':
...
//http://www.cnblogs.com/hourglasser/archive/2013/08/03/3231548.html
$.fn.extend({
    uploadfile: function (options) {
        ...
        try {
            if (!options.view) {
                $this.uploadify({
                    swf: options.applicationPath + '/Plugins/jquery-uploadify/uploadify.swf', //swf文件件地址
                    uploader: options.applicationPath + '/Handlers/UploadFilesHandler.ashx', //设置文件接收的动态页路径
                    fileObjName: 'Filedata', //指定在服务器处理程序中根据该名字来取上传文件的数据。默认为Filedata
                    ...
                    formData: { 'RefGuid': options.refGuid, 'Type': options.type, 'FileSign': options.fileSign, 'extend': options.extend },
                    onUploadProgress: function (file, bytesUploaded, bytesTotal, totalBytesUploaded, totalBytesTotal) {
                        if (options.progress) {
                            var w = Math.round((bytesUploaded / bytesTotal) * 100);
                            $('#' + options.content).find("#pro").html("<div style='width:" + w + "%;background-color:lightgreen;float:left;height:10px'>&nbsp;</div>");
                        }
                    },
                    onUploadStart: function (file) {
                        if (options.progress) {
                            $('#' + options.content).find("#pro").remove();
                            var x = -32, y = -32;
                            $div = $("<div/>").css({ "border-radius": "5px", "background-color": "white", "border": "1px solid lightblue", "width": "309px", "height": "44px", "line-height": "16px", "float": "left", "margin": "0 5px 5px 0" });
                            file.type = file.type.toLowerCase();
                            var cls = GetFileType(file.type.toLowerCase());
                            $img = $("<b />", { "class": cls });
                            $div.append($img);
                            $divTitle = $("<div />");
                            $divTitle.html(file.name).attr("title", file.name);
                            $TitleContent = $("<div />").css({ "float": "right", "width": "260px" });
                            $div.append($TitleContent);
                            $TitleContent.append($divTitle);
                            $($divTitle).css({ "width": "260px", "height": "16px", "color": "#666", "overflow": "hidden", "white-space": "nowrap", "text-overflow": "ellipsis", "margin": "5px 0 0 0", "font-size": "11px" });
                            $('#' + options.content).append($div);
                            $divProgress = $("<div  />", { "id": "divProgress" }).css({ "float": "right", "width": "260px", "height": "16px", "color": "#666", "overflow": "hidden", "white-space": "nowrap", "text-overflow": "ellipsis", "margin": "5px 0 0 0", "font-size": "11px" });
                            $divTitle.after($divProgress);
                            var re = "M";
                            var num = (file.size / 1024 / 1024).toFixed(2);
                            if (num < 1) {
                                num = (file.size / 1024).toFixed(2); re = "K";
                            }
                            if (num >= 1024) {
                                num = (file.size / 1024 / 1024 / 1024).toFixed(2); re = "G";
                            }
                            $pro = $("<div />", { "id": "pro", "class": "filepro" }).css({ "width": 200, "height": "10px", "float": "left", "background": "white" });
                            $divProgress.append($pro);
                            $divProgress.append("&nbsp;&nbsp;" + num.replace("00", "") + re);
                            $('#' + options.content).find("div[style*='clear']").remove(); $('#' + options.content).append('<div style="clear:both"></div>');
                        }
                    },
                    onUploadSuccess: function (file, data, response) {
                        ...
                        if (options.progress) {
                            ...
                            $('#' + options.content).find("[title='" + file.name + "']").html("<div style='float:left;width:200px;overflow:hidden;white-space:nowrap;white-space:ellipsis;'><a style=\"text-decoration:none;color:#666;cursor:pointer;margin-right:10px;\" href='" + options.applicationPath + "/Handlers/UploadFilesHandler.ashx?type=download&Guid=" + json[0].Guid + "'>" + file.name + "</a></div>");
                            $('#' + options.content).find("[title='" + file.name + "']").append("<a style=\"float:right;cursor:pointer;margin-right:5px\" guid='" + json[0].Guid + "' >删除</a>");
                        }
                        file.guid = json[0].Guid;
                        if (json[0].FileNewName)
                            file.FileNewName = json[0].FileNewName;
                        options.success(file);
                    },
                    onUploadError: function (file, errorCode, errorMsg, errorString) {
                        if (options.progress) {
                            $('#' + options.content).find("[title='" + file.name + "']").html("<div style='float:left;width:200px;overflow:hidden;white-space:nowrap;white-space:ellipsis;'><a style=\"text-decoration:none;color:#666;cursor:pointer;margin-right:10px;\"><font color='red'>[上传失败]</font>" + file.name + "</a></div>");
                            $('#' + options.content).find("[title='" + file.name + "']").append("<a style=\"float:right;cursor:pointer;margin-right:5px\" delete >删除</a>");
                        }
                        options.error(file);
                    }
                });
            ...
            $.ajax({
                ...
                type: "post",
                async: false,
                data: { type: 'get', 'RefGuid': options.refGuid, 'FileSign': options.fileSign, 'extend': options.extend },
                success: function (data) {
                    var json = eval(decodeURIComponent(data));
                    $.each(json, function (i, dat) {
                        $div = $("<div/>").css({ "border-radius": "5px", "background-color": "white", "border": "1px solid lightblue", "width": "309px", "height": "44px", "line-height": "16px", "float": "left", "margin": "0 5px 5px 0" });
                        var cls = GetFileType(dat.FileExtName.toLowerCase());
                        $img = $("<b />", { "class": cls });
                        $div.append($img);
                        $divTitle = $("<div />");
                        $divTitle.html(dat.OrginFileName).attr("title", dat.OrginFileName);
                        $TitleContent = $("<div />").css({ "float": "right", "width": "260px" });
                        $div.append($TitleContent);
                        $TitleContent.append($divTitle);
                        $($divTitle).css({ "width": "260px", "height": "16px", "color": "#666", "overflow": "hidden", "white-space": "nowrap", "text-overflow": "ellipsis", "margin": "5px 0 0 0", "font-size": "11px" });
                        ...
                        $divTitle.after($divProgress);
                        var re = "M";
                        var num = (dat.FileSize / 1024 / 1024).toFixed(2);
                        if (num < 1) {
                            num = (dat.FileSize / 1024).toFixed(2); re = "K";
                        }
                        if (num >= 1024) {
                            num = (dat.FileSize / 1024 / 1024 / 1024).toFixed(2); re = "G";
                        }

                        $divProgress.append("&nbsp;&nbsp;" + num.replace("00", "") + re);
                        $('#' + options.content).find("[title='" + dat.OrginFileName + "']").html("<div style='float:left;width:200px;overflow:hidden;white-space:nowrap;white-space:ellipsis;'><a style=\"text-decoration:none;color:#666;cursor:pointer;margin-right:10px;\" href='" + options.applicationPath + "/Handlers/UploadFilesHandler.ashx?type=download&Guid=" + dat.Guid + "'>" + dat.OrginFileName + "</a></div>");
                        if (!options.view)
                            $('#' + options.content).find("[title='" + dat.OrginFileName + "']").append("<a style=\"float:right;cursor:pointer;margin-right:5px\" guid='" + dat.Guid + "' >删除</a>");
                        $('#' + options.content).find("div[style*='clear']").remove(); $('#' + options.content).append('<div style="clear:both"></div>');
                    });
                },
                error: function (data) {
                    alert("获取附件失败" + data.status);
                }
            });
            if (options.view) {
                $('#' + $this.attr("id")).remove();
                ...
                try {
                    $("#" + $this.attr("id") + "_button").removeClass("uploadify-button").addClass(options.btnCls).html(options.title);
                    $("#" + $this.attr("id") + "_queue").remove();
           
                }
                catch (ex)
                { }
            }
            $("#" + options.content + " a[guid]").live("click", function () {
                var $filea = $(this);
                if (confirm("确定删除附件吗")) {
                    //alert("in");
                    var guid = $filea.attr("guid");
                    $.ajax({
                        url: options.applicationPath + '/Handlers/UploadFilesHandler.ashx',
                        type: "post",
                        asnyc: false,
                        data: { type: "delete", guid: guid },
                        success: function (data) {
                            $filea.parent().parent().parent().remove();
                        },
                        error: function () {
                            alert("删除失败");
                        }
            ...
            function GetFileType(type) {
                ...
                if (type == ".doc" || type == ".docx")
                    ...
                    cls = "fileflash";
                else if (type == ".rmvb" || type == ".avi" || type == ".mp4" || type == ".mkv")
                    cls = "filevideo";
                else if (type == ".jpg")
                    cls = "filejpg";
                else if (type == ".gif")
                    ...
                    cls = "fileico";
                else if (type == ".txt" || type == ".chm")
                    cls = "filetxt";
                else if (type == ".html" || type == ".htm" || type == ".asp" || type == ".aspx" || type == ".jsp" || type == ".cshtml" || type == ".vbhtml")
                    cls = "filehtml";
                return cls;
            }
    ...
    fileviewload: function (options) {
        ...
        $.ajax({
            ...
            success: function (data) {
                var json = eval(decodeURIComponent(data));
                $.each(json, function (i, dat) {
                    $div = $("<div/>").addClass("filecontent").attr("title", dat.OrginFileName);

                    var cls = GetFileType(dat.FileExtName.toLowerCase());
                    $img = $("<b />", { "class": cls });
                    var $a = $("<a />").css("cursor", "opointer").attr("href", options.applicationPath + "/Handlers/UploadFilesHandler.ashx?type=load&Guid=" + dat.Guid);
                    $a.append($img);
                    $div.append($a);
                    $divTitle = $("<div />").css("margin", "7px 0 5px 0").addClass("filetitle");
                    $divTitle.html("<a style='text-decoration:none;color:black'  href='" + options.applicationPath + "/Handlers/UploadFilesHandler.ashx?type=load&Guid=" + dat.Guid + "'>" + subFileName(dat.OrginFileName) + "</a>");
                    ...
                    var $sizediv = $("<div />").addClass("filesize");
                    $sizediv.html(num.replace("00", "") + re);
                    if (options.deleted) {
                        var $dimg = $("<img />").attr("src", options.applicationPath + "/Plugins/jquery-uploadify/cross.png");
                        $dimg.css({ "width": "12px", "height": "12px", "margin-left": "10px", "vertical-align": "center", "cursor": "pointer" });
                        $dimg.attr("title", "删除附件");
                        $dimg.click(function () {
                            ...
                            if (confirm("确定删除附件吗")) {
                                ...
                                $.ajax({
                                    ...
                                    type: "post",
                                    asnyc: false,
                                    data: { type: "delete", guid: dat.Guid },
                                    success: function (data) {
                                        $div.remove();
                                    },
                                    error: function () {
                    ...
                    $div.append($sizediv);
                    $this.append($div);
                    $this.find("div[style*='clear']").remove(); $this.append('<div style="clear:both"></div>');
                });
            },
            error: function (data) {
                alert("获取附件失败" + data.status);
