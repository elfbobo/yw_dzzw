             ...
             //   SetTaleEquel(headerTable, table);
                ...
                function GetData(idField, ajaxUrl, sort, order, connectionName, connectionString, providerName, tableName, condition, pageCount, where, columns, page) {
                    $.ajax({
                        ...
                        data: { idField: idField, sort: sort, order: order, connectionName: connectionName, connectionString: connectionString, providerName: providerName, tableName: tableName, condition: encMe(condition, 'asdefzgereagjlg'), rows: pageCount, page: page, where: encMe(where, 'asdefzgereagjlg') },
        ...
        DetelteRow: function (url) {
            ...
            if (confirm("确定要删除吗")) {
                ...
                $.ajax({
                    url: url,
                    type: "post",
                    data: { tableName: cfg.tableName, where: encMe(" and proguid in('" + guids.join("','") + "')", 'asdefzgereagjlg') },
                    dataType: "text",
